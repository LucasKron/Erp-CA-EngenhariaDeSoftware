const STATUS_CONF = {
  planejado: { label: 'Planejado',    badge: 'badge-blue',   icon: '📋' },
  andamento: { label: 'Em andamento', badge: 'badge-yellow', icon: '🔄' },
  concluido: { label: 'Concluído',    badge: 'badge-green',  icon: '✅' },
...
function renderStats() {
  ...
  const stats = [
    { label: 'Total',        value: ev.length,                                         icon: '📅', color: 'icon-blue'   },
    { label: 'Planejados',   value: ev.filter(e => e.status === 'planejado').length,   icon: '📋', color: 'icon-purple' },
    { label: 'Em andamento', value: ev.filter(e => e.status === 'andamento').length,   icon: '🔄', color: 'icon-yellow' },
    { label: 'Concluídos',   value: ev.filter(e => e.status === 'concluido').length,   icon: '✅', color: 'icon-green'  }
  ];
...
function renderEventos() {
  ...
  if (!eventos.length) {
    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📅</div><h3>Nenhum evento encontrado</h3><p>Adicione eventos para acompanhar a agenda do CA.</p><button type="button" class="btn btn-primary" onclick="abrirModalEvento()">+ Novo Evento</button></div>';
    return;
  }
  ...
  eventos.forEach(e => {
    ...
    html += '<span class="badge ' + status.badge + '">' + status.icon + ' ' + status.label + '</span></div>';
    html += '<div class="event-details">' + details + '</div>';
    if (e.descricao) html += '<div style="font-size:13px;color:#718096;line-height:1.5;margin-top:2px">' + (e.descricao.length > 120 ? e.descricao.slice(0, 120) + '…' : e.descricao) + '</div>';
    html += '</div>';
    html += '<div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">';
    html += '<button type="button" class="btn btn-outline btn-sm btn-icon" onclick="event.stopPropagation();abrirModalEvento(\'' + e.id + '\')">✏</button>';
    html += '<button type="button" class="btn btn-danger btn-sm btn-icon" onclick="event.stopPropagation();excluirEvento(\'' + e.id + '\')">🗑</button>';
    html += '</div></div>';
  });
  html += '</div>';
...
function verEvento(id) {
  ...
  let body = '<div style="display:flex;gap:12px;align-items:center;margin-bottom:16px">';
  body += '<span class="badge ' + status.badge + '" style="font-size:13px;padding:5px 12px">' + status.icon + ' ' + status.label + '</span>';
  body += '<span style="font-size:14px;font-weight:700;color:#1A202C">' + dataFmt + '</span></div>';
  ...
  if (e.descricao) body += '<hr class="divider"><div style="font-size:13.5px;color:#4A5568;line-height:1.7;white-space:pre-wrap">' + e.descricao + '</div>';
  if (e.observacao) body += '<hr class="divider"><div style="font-size:12.5px;color:#718096;background:#F7FAFC;border-radius:8px;padding:12px;font-style:italic">' + e.observacao + '</div>';
